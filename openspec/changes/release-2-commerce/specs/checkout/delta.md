...
> **References**: [Checkout specification](../../../../specs/checkout/spec.md)
> **Change**: Implements a multi-step checkout form wizard with Zod validation, stock-atomic order creation via Server Action, and preference-expired cleanup.

## Status

| REQ ID | Requirement | Priority | Scenarios | PR |
|--------|------------|----------|-----------|-----|
| REQ-CHK-01 | Multi-Step Form Wizard | P0 | SC-CHK-01, SC-CHK-02 | 2 |
| REQ-CHK-02 | Form Validation (Zod) | P0 | SC-CHK-03, SC-CHK-04 | 2 |
| REQ-CHK-03 | Customer Information | P0 | SC-CHK-05, SC-CHK-06 | 2 |
| REQ-CHK-04 | Shipping Address | P0 | SC-CHK-07, SC-CHK-08 | 2 |
| REQ-CHK-05 | Order Summary | P0 | SC-CHK-09 | 2 |
| REQ-CHK-06 | Order Creation (Server Action) | P0 | SC-CHK-10, SC-CHK-11 | 2 |
| REQ-CHK-07 | Stock Validation | P0 | SC-CHK-12, SC-CHK-13 | 2 |
| REQ-CHK-08 | Expired Preference Cleanup | P1 | SC-CHK-14 | 2 |
| REQ-CHK-09 | Empty Cart Redirect | P0 | SC-CHK-15 | 2 |
| REQ-CHK-10 | Error Handling | P0 | SC-CHK-16, SC-CHK-17 | 2 |

**Total**: 10 requirements | 17 scenarios | Budget: PR 2 (~400 lines)
...
(Previously: customer info collection was specified without concrete field definitions or Zod validation)
...
(Previously: shipping info was specified without Zod validation)
...
(Previously: summary was specified without live cart integration)
...
(Previously: order creation was specified without implementation details — transaction, stock WHERE clause, cart clearing)
...
(Previously: stock validation was specified generically — now uses `prisma.$transaction` with `WHERE stock >= quantity` clause)
...
(Previously: not specified in checkout spec — handled in cart spec)
...
(Previously: error handling was not specified)
...
None. The MercadoPago preference creation and redirect are detailed in the payments delta. The checkout spec's existing requirements for "Mercado Pago Preference Creation" and "Redirect to Mercado Pago" remain valid as cross-domain workflow steps but their implementation details are owned by the payments module.
...
|----|-------|-------------|
| 2 | Checkout + Orders | REQ-CHK-01 through REQ-CHK-10 (checkout form + order creation) |
